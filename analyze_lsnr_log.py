...
parser.add_option("-f", "--file", dest="filename",
                  default=None,
                  help="Name of the listener logfile. When specified, we won't attempt to query the running listener "
                       "process.")
...
output_format = pip._vendor.distlib.compat.raw_input()
    ...
    for d in date_list:
        ...
        for p in prog_list:
            if dict_program.get((d, p), None):
                print("," + str(dict_program[d, p]), end=' ')
            else:
                print(",", end=' ')
        print()
elif output_format == "2":
    for i in range(len(prog_list) + 1):
        print("|".ljust(col_width + 2, "="), end=' ')
    print("|")
    print("| Date".ljust(col_width + 2), end=' ')
    for p in prog_list:
        print("| " + p.ljust(col_width)[:col_width], end=' ')
    print("|")
    for i in range(len(prog_list) + 1):
        print("|=".ljust(col_width + 2, "="), end=' ')
    print("|")
    for d in date_list:
        print("| " + d.strftime("%Y-%m-%d").ljust(col_width)[:col_width], end=' ')
        for p in prog_list:
            if dict_program.get((d, p), None):
                print("| " + str(dict_program[d, p]).ljust(col_width)[:col_width], end=' ')
            else:
                print("|".ljust(col_width + 2), end=' ')
        print("|")
    for i in range(len(prog_list) + 1):
